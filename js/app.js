...
function min_heapify() {
    ...
    if (inputText.value !== '') {
      ...
      createBinaryTreeAndArr(input);
      document.getElementById('instructions').innerHTML = "<p> Parent's value is always less than or equal to the values of its children.</p>";
...
document.getElementById("add").addEventListener("click", function () {
    const addInput = document.getElementById("addElements").value;
    const addElement = parseInt(addInput);
    if (!isNaN(addElement)) {
        input.push(addElement);  // Add element to the array
        updateTree();
        console.log(input);
...
document.getElementById("remove").addEventListener("click", function () {
    const removeInput = document.getElementById("removeElements").value;
    const removeElement = parseInt(removeInput);
    if (!isNaN(removeElement)) {
        input = input.filter(item => item !== removeElement);  // Remove element from array
        updateTree();     
        console.log(input);
                                              // Update the visualizer
    } else {
